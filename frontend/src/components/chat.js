    ...
    const makeApiCall = async () => {
      ...
      try {
        ...
        if (selectedFile && summaryText === '') {
          ...
          setMessages([...messages, data.summary]);
    ...
    return (
      <Container>
        <div className="d-flex justify-content-center align-items-center vh-50">
          ...
          {messages && messages.length===0 && 
            <Paper square className='bg-dark text-white' elevation={3} style={{ padding: '16px', marginBottom: '16px' }}>
              <Typography className="text-center" variant="h5">Submit a Document (PDF, DOCX, TXT) for Summary and Q&A </Typography>
            </Paper>}
        ...
        <InputGroup>
          <FormControl
            className="rounded-0"
            placeholder={summaryText.length>0  ? "Ask a Question about the document..." : "Please upload a document to start Q&A"}
            as="textarea"
            value={inputText}
            onChange={handleInputChange}
            disabled={summaryText === ''}
          />
          <Button className="sm rounded-0" onClick={makeApiCall} disabled={summaryText === '' || (inputText.length===0)}>
            <SendIcon />
          </Button>
        </InputGroup>
      </Container>
